...
/// Generate a string with the version and git hash if available
/// If the git hash is not available, only the version is returned
pub fn get_version_digits() -> String {
  let semver = option_env!("CARGO_PKG_VERSION").unwrap_or("unknown");
  match option_env!("LUMOS_SVM_BUILD_GIT_HASH") {
...
mod tests {
  ...
  #[test]
  fn it_should_get_version_digits_no_hash() -> anyhow::Result<()> {
  ...
  #[test]
  fn it_should_get_version_digits_format() -> anyhow::Result<()> {
